    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative dk-surface rounded-3xl w-full max-w-lg p-6 shadow-2xl space-y-5" style={{ maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3.5 rounded-2xl dk-surface-2 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {form.imageUrl && (
                <button
                  ...
                  className="text-[11px] text-red-500 dark:text-red-400 hover:underline flex items-center gap-1 font-medium"
                >
            ...
            <div className="flex items-center gap-4">
              {/* Thumbnail Preview */}
              <div className="h-20 w-20 shrink-0 rounded-xl dk-surface flex items-center justify-center overflow-hidden relative shadow-inner">
                {form.imageUrl ? (
              ...
              <div className="flex-1 space-y-2">
                <label className="cursor-pointer dk-surface hover:border-cyan-500/50 rounded-xl px-3 py-2 text-xs font-semibold flex items-center justify-center gap-2 transition-all shadow-sm" style={{ color: "var(--text)" }}>
                  <Upload className="h-3.5 w-3.5 text-cyan-500 dark:text-cyan-400" />
          ...
          <div>
            <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>Nom du produit</label>
            <input
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>Catégorie</label>
              <select
                value={form.category}
            ...
            <div>
              <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>État du Stock</label>
              <select
                value={form.stock}
                onChange={(e) => setForm({ ...form, stock: e.target.value as StockStatus })}
                className="dk-input rounded-xl px-3.5 py-2.5 w-full text-sm"
              >
                <option value="in" className="bg-[var(--surface)] text-[var(--text)]">En stock (Disponible)</option>
                <option value="low" className="bg-[var(--surface)] text-[var(--text)]">Stock limité (&lt; 5)</option>
                <option value="out" className="bg-[var(--surface)] text-[var(--text)]">Rupture de stock (0)</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>Prix de vente (DA)</label>
              <input
                type="number"
                value={form.price}
            ...
            <div>
              <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>Ancien prix (Biffé)</label>
              <input
                type="number"
                value={form.oldPrice}
          ...
          <div>
            <label className="text-xs font-bold block mb-1" style={{ color: "var(--text)" }}>Description courte</label>
            <textarea
              rows={2}
        ...
        <div className="flex gap-3 pt-3 border-t" style={{ borderColor: "var(--border)" }}>
          <button onClick={() => onSave(form)} className="flex-1 rounded-xl py-3 text-sm font-bold dk-btn-primary transition-all shadow-md">
            {initial ? "Mettre à jour" : "Ajouter au catalogue"}
          </button>
          <button onClick={onClose} className="rounded-xl px-5 py-3 text-sm font-semibold dk-btn-secondary transition-colors">
            Annuler
          </button>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="dk-btn-primary font-bold text-xs sm:text-sm px-5 py-2.5 rounded-xl shadow-md flex items-center justify-center gap-2 transition-all"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody className="divide-y" style={{ borderColor: "var(--border)" }}>
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-[var(--surface-2)]/60 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="h-11 w-11 shrink-0 rounded-xl overflow-hidden dk-surface-2 flex items-center justify-center p-1 shadow-sm">
                        {p.imageUrl ? (
                      ...
                      <div>
                        <p className="font-bold leading-tight" style={{ color: "var(--text)" }}>{p.name}</p>
                        <p className="text-[11px] line-clamp-1 max-w-xs" style={{ color: "var(--text-dim)" }}>{p.shortDesc}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap font-mono font-bold" style={{ color: "var(--text)" }}>
                    {formatDZD(p.price)}
                    {p.oldPrice && <span className="text-[10px] line-through block font-normal" style={{ color: "var(--text-faint)" }}>{formatDZD(p.oldPrice)}</span>}
                  </td>
