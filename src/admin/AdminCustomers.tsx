    ...
    if (c.totalSpent >= 10000 || c.orders.length >= 3) {
      return (
        <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
          <Star className="h-3 w-3 fill-amber-400 text-amber-400" /> Client VIP
        </span>
      );
    }
    if (c.orders.length >= 2) {
      return (
        <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
          <Award className="h-3 w-3 text-cyan-400" /> Fidèle ({c.orders.length} cmd)
        </span>
      );
    }
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="dk-input w-full pl-10 pr-4 py-2.5 rounded-xl text-xs sm:text-sm bg-slate-900/90 text-white"
        />
      </div>

      {/* Customer CRM Table */}
      <div className="dk-surface rounded-2xl overflow-hidden border border-slate-700/80 bg-[#1a2235] shadow-xl">
            ...
            <tbody className="divide-y divide-slate-800/80">
              {filteredCustomers.map((c) => (
                <tr key={c.phone} className="hover:bg-slate-900/40 transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap font-semibold text-slate-300">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-right">
                    <button
                      ...
                      className="h-8 px-3 rounded-lg bg-slate-800 hover:bg-cyan-500/20 text-slate-300 hover:text-cyan-400 text-xs font-bold transition-colors inline-flex items-center gap-1.5"
                    >
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative dk-surface rounded-3xl w-full max-w-lg p-6 border border-slate-700/80 bg-[#1a2235] shadow-2xl space-y-5" style={{ maxHeight: "90vh", overflowY: "auto" }}>
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setSelectedCustomer(null)} className="h-8 w-8 rounded-full bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
                <p className="text-xs text-slate-400">Valeur Totale (LTV)</p>
                <p className="text-lg font-extrabold font-mono text-cyan-400 mt-1">{formatDZD(selectedCustomer.totalSpent)}</p>
              </div>
              <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
                <p className="text-xs text-slate-400">Statut Client</p>
            ...
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2 text-xs text-slate-300">
              <p className="font-bold text-white">Coordonnées de livraison</p>
              <p className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-slate-500" /> {selectedCustomer.phone}</p>
              <p className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-slate-500" /> Wilaya de {selectedCustomer.wilaya}</p>
            </div>

            {/* Order History */}
            <div className="space-y-2">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Historique des Commandes ({selectedCustomer.orders.length})</p>
              <div className="space-y-2">
                {selectedCustomer.orders.map((o) => (
                  <div key={o.id} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-between text-xs">
                    <div>
                      <p className="font-bold text-cyan-400">{o.id} • {o.date}</p>
                      <p className="text-slate-400 text-[11px]">{o.items.map((i) => `${i.name} (x${i.qty})`).join(", ")}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-mono font-bold text-white">{formatDZD(o.total)}</p>
                      <span className="text-[10px] text-cyan-300">{o.status}</span>
                    </div>
                  </div>
                ))}
              </div>
